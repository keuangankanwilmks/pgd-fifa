...
import { Search, Filter, Download, Edit2, Trash2, Calendar, Landmark, CheckCircle, AlertCircle, RefreshCw, Play, ChevronLeft, ChevronRight } from 'lucide-react';
...
export function DataRekon({ bank, onUpdateRekon, currentUser }: DataRekonProps) {
  ...
  const confirmDelete = async () => {
    ...
    const loadingToast = toast.loading('Menghapus data dari Google Sheets...');
    setIsLoading(true);
    const spreadsheetId = import.meta.env.VITE_REKON_SPREADSHEET_ID;
    ...
    try {
      ...
      console.log(`[Delete] Target Row Index: ${rowToDelete}`);
      ...
      console.log(`[Delete] Sheet ID: ${sheetId}`);
      
      const result = await googleSheetsService.deleteRow(spreadsheetId, sheetId, rowToDelete);
      console.log('[Delete] Success result:', result);
      
      toast.success('Data berhasil dihapus dari Google Sheets', { id: loadingToast });
      setIsConfirmOpen(false);
      await fetchData();
    } catch (error: any) {
  ...
  return (
    <div className="space-y-3">
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
        ...
        <div className="overflow-x-auto h-[calc(100vh-280px)] border border-gray-100 rounded-xl shadow-inner bg-gray-50/30">
            ...
            <tbody className="divide-y divide-gray-100 bg-white">
              {paginatedData.length > 0 ? (
                paginatedData.map((item, i) => {
                  ...
                  return (
                    <tr key={item.id} className={`transition-colors group ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'} hover:bg-emerald-50/40`}>
      ...
      <ConfirmModal
        ...
        message="Apakah Anda yakin ingin menghapus data ini dari Google Sheets? Tindakan ini tidak dapat dibatalkan."
