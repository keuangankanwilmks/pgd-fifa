...
export function DataRekon({ bank, onUpdateRekon, currentUser, roleDatabasePermissionMap = {} }: DataRekonProps) {
  ...
  const filteredData = useMemo(() => {
    return savedData.filter((item: any) => {
      ...
      return matchBank && matchSearch && matchCabang && matchStatus && matchDate;
    });
  }, [savedData, bank, searchTerm, selectedCabang, selectedStatus, startDate, endDate]);

  // Reset to page 1 when filters change
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, selectedCabang, selectedStatus, startDate, endDate, itemsPerPage]);
  ...
  const statusOptions = useMemo(() => {
    const uniqueStatus = Array.from(new Set(
      savedData
        .filter((item: any) => item.bank === bank)
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">Kelola hasil rekonsiliasi yang telah tersimpan.</p>
              </div>
            </div>
          ...
          <table className="w-full text-[12px] border-collapse min-w-[1200px]">
            <thead className="sticky top-0 z-20">
              <tr className="bg-[#005245] border-b border-[#004237]">
                ...
                <th className="text-left py-1.5 px-5 font-black text-white uppercase text-[9px] tracking-widest border-r border-[#004237]/50">Keterangan</th>
                ...
                <th className="text-right py-1.5 px-5 font-black text-white uppercase text-[9px] tracking-widest border-r border-[#004237]/50">Nominal Sistem</th>
                <th className="text-right py-1.5 px-5 font-black text-white uppercase text-[9px] tracking-widest border-r border-[#004237]/50">Nominal Bank</th>
                <th className="text-right py-1.5 px-5 font-black text-white uppercase text-[9px] tracking-widest border-r border-[#004237]/50">Selisih</th>
                <th className="text-center py-1.5 px-5 font-black text-white uppercase text-[9px] tracking-widest border-r border-[#004237]/50">Status</th>
                <th className="text-center py-1.5 px-5 font-black text-white uppercase text-[9px] tracking-widest border-r border-[#004237]/50">Kategori</th>
                <th className="text-left py-1.5 px-5 font-black text-white uppercase text-[9px] tracking-widest border-r border-[#004237]/50">Catatan Rekon</th>
                ...
                paginatedData.map((item, i) => {
                  ...
                  return (
                    <tr key={item.id} className={`transition-colors group ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'} hover:bg-emerald-50/40 ${selectedRows.includes(item.rowIndex) ? 'bg-emerald-50' : ''}`}>
                      ...
                      <td className="py-0.5 px-5 text-gray-800 font-bold max-w-md truncate group-hover:whitespace-normal group-hover:overflow-visible group-hover:bg-white group-hover:shadow-xl group-hover:z-10 group-hover:relative transition-all border-r border-gray-50">{item.keterangan}</td>
                      <td className="py-0.5 px-5 border-r border-gray-50">
                        <span className={`px-2 py-0.5 rounded text-[9px] font-black tracking-wider shadow-sm ${bankColor}`}>
                          {item.bank}
                        </span>
                      </td>
                      <td className="py-0.5 px-5 text-gray-700 font-bold border-r border-gray-50">{item.cabang}</td>
                      <td className="py-0.5 px-5 text-right font-mono text-gray-900 font-black border-r border-gray-50 text-[11px]">{formatCurrency(item.nominalSistem)}</td>
                      <td className="py-0.5 px-5 text-right font-mono text-gray-900 font-black border-r border-gray-50 text-[11px]">{formatCurrency(item.nominalBank)}</td>
                      <td className="py-0.5 px-5 text-right font-mono text-gray-900 font-black border-r border-gray-50 text-[11px]">{formatCurrency(item.selisih)}</td>
                      <td className="py-0.5 px-5 text-center border-r border-gray-50">
                        <div className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-sm ${
                      ...
                      <td className="py-0.5 px-5 border-r border-gray-50">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-sm ${item.kategori === 'Non Pembayaran' ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-blue-700'}`}>
                          {item.kategori}
                        </span>
                      </td>
                      <td className="py-0.5 px-5 border-r border-gray-50">
