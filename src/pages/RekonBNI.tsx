...
export function RekonBNI({ 
  ...
  const parseSistem = (data: any[][]) => {
    ...
    for (let i = 12; i < data.length; i++) {
      ...
      if (tanggalRaw && String(tanggalRaw).trim() !== "" && 
        ...
        let namaCabang = getCabang(keteranganFinal);
  ...
  const getCabang = (keterangan: string) => {
    ...
    // This logic looks for the word immediately following the keyword
    ...
    if (match && match[1]) {
      ...
      // Try matching the first few words against Firestore namaCabang
      for (let k = 0; k < Math.min(words.length, 3); k++) {
        const word = words[k].replace(/[^A-Z0-9]/g, '');
        if (word.length < 3) continue;
        
        for (const mapping of norekMappings) {
          const targetCabang = mapping.namaCabang.toUpperCase();
          if (targetCabang.includes(word) || word.includes(targetCabang)) {
            return mapping.namaCabang;
          }
        }
      }
      
      // If no match in Firestore, format it as "CP [WORD]" as a best guess
        ...
        <div className="flex-1 grid grid-cols-2 gap-6">
          ...
          <div className="col-span-2 flex justify-end mt-4">
            <button 
              ...
              className={`px-8 py-4 rounded-xl font-bold flex items-center gap-2 transition-all ${sistemFile && bankFile ? 'bg-[#009B4F] hover:bg-[#008543] text-white shadow-md shadow-[#009B4F]/20 cursor-pointer' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}
            >
