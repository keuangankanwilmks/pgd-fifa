  ...
  async getAll(): Promise<Cabang[]> {
    ...
    try {
      ...
      if (snapshot.empty) {
        try {
          await this.seed();
          const newSnapshot = await getDocs(collection(db, path));
          return newSnapshot.docs.map(doc => ({ 
            id: doc.id, 
            ...doc.data() as any
          }));
        } catch (seedError) {
          console.warn('Failed to seed cabang data (likely guest user), falling back to local data:', seedError);
          return CABANG_DATA.map((item, index) => ({
            id: item.sapCode || `local_${index}`,
            ...item
          }));
        }
      }
      ...
      // Fallback to local data on any error (including permission denied)
      return CABANG_DATA.map((item, index) => ({
        id: item.sapCode || `local_${index}`,
        ...item
      }));
